...
class Binary
{
     ...
     * @var string
     */
    public const STREAM_NAME = 'mailsobinary';
    ...
    public static function CreateStream(
        $rStream,
        $sUtilsDecodeOrEncodeFunctionName = null,
        $sFromEncoding = null,
        $sToEncoding = null
    ) {
        ...
        if (in_array($sUtilsDecodeOrEncodeFunctionName, array(
            ...
            $rFilter = \stream_filter_append(
                $rStream,
                $sUtilsDecodeOrEncodeFunctionName,
                STREAM_FILTER_READ,
                array(
            ...
            if ($iCount < strlen($this->sBuffer)) {
                ...
                while ($iCount > 0) {
                    if (feof($this->rStream)) {
                        ...
                        $sReturn .= self::$sFunctionName(
                            $this->sReadEndBuffer.$sReadResult,
                            $this->sReadEndBuffer,
                            $this->sFromEncoding,
                            $this->sToEncoding
                        );
